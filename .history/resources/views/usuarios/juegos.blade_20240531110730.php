...
<body>
    ...
    <div class="container">
        <div class="filter">
            <label>
                <input type="radio" name="filter" value="all" checked onclick="filterGames('all')">
                Todos los juegos
            </label>
            <label>
                <input type="radio" name="filter" value="favorites" onclick="filterGames('favorites')">
                Juegos favoritos
            </label>
            <label>
                <input type="radio" name="filter" value="recommended" onclick="filterGames('recommended')">
                Juegos recomendados
            </label>
        </div>
        <h1>Todos los juegos</h1>
        @foreach($juegos as $juego)
            <div class="juego {{ in_array($juego->id, $usuario->favoritos) ? 'favorite' : '' }}">
                <h2>{{ $juego->nombre }}</h2>
                <div class="imagen">
    ...
    <script>
        ...
        function filterGames(filter) {
            ...
            juegos.forEach(function(juego) {
                if (filter === 'all') {
                    juego.classList.add('all');
                } else if (filter === 'favorites') {
                    if (juego.classList.contains('favorite')) {
                        juego.classList.add('favorite');
                        juego.classList.remove('all', 'recommended');
                    } else {
                        juego.classList.remove('favorite', 'all', 'recommended');
                    }
                } else if (filter === 'recommended') {
                    juego.classList.remove('all', 'favorite');
                    // Lógica adicional para juegos recomendados
                }
            });
        }
